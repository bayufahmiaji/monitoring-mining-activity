        ...
        <table table id="example3" class="display table table-stripped table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modaledit" data-lokasi="{{$transport->lokasi}}" data-jumlah="{{$transport->jumlah}}" data-lokasi="{{$transport->lokasi}}" data-date="{{$transport->date}}" data-jarak="{{$transport->jarak}}" data-biaya="{{$transport->biaya}}" data-id="{{$transport->id}}" > <i class="fa fa-edit"></i></button>
                    <a href="/transport/{{$transport->id}}/delete" class="btn btn-danger" onclick="return confirm('Delete Pengangkutan?')"><i class="fa fa-trash"></i></a></td>
                </tr>
               @endforeach
            </tbody>
            <tfoot>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tambah Pengangkutan Tambang</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form method="post" action="/transport/add" name="hitung">
          {{csrf_field()}}
          <input type="hidden" name="jenis" value="Pengangkutan">
        ...
        <input type="hidden" name="company" value="{{auth()->user()->name}}">
          <div class="form-group">
              <label for="message-text" class="col-form-label">Jumlah (Ton)</label>
              <input class="form-control" type="text" placeholder="jumlah" name="jumlah" onchange="proses()">
          </div>
          <div class="form-group">
              <label for="message-text" class="col-form-label">Jarak (KM)</label>
              <input class="form-control" type="text" placeholder="jarak" name="jarak" onchange="proses()">
          </div>
           <div class="form-group">
              <label for="message-text" class="col-form-label">Biaya Pengangkutan ($)</label>
...
<div class="modal fade" id="modaledit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Edit Pengangkutan Tambang</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form method="post" action="/transport/update" name="hitung2">
          {{csrf_field()}}
        ...
        <input type="hidden" name="company" value="{{auth()->user()->name}}">
          <div class="form-group">
              ...
              <input class="form-control" type="text" placeholder="jumlah" name="jumlah" id="jumlah" onchange="process()">
          </div>
          <div class="form-group">
              <label for="message-text" class="col-form-label">Jarak</label>
              <input class="form-control" type="text" placeholder="jarak" name="jarak" id="jarak" onchange="process()">
          </div>
           <div class="form-group">
              <label for="message-text" class="col-form-label">Biaya Pengangkutan</label>
              <input class="form-control" type="text" readonly name="biaya" id="biaya">
          </div>
          <div class="form-group">
            <label for="message-text" class="col-form-label">Lokasi</label>
            <select class="form-control" id="exampleFormControlSelect1" name="lokasi" id="lokasi">
                @foreach($asu as $asu)
...
@section('footer_scripts')
   <!--plugin scripts-->
    ...
    var jarak =parseFloat(myForm.jarak.value);
    if(myForm.jumlah.value == "") jumlah=0;
    if(myForm.jarak.value == "") jarak=0;
    myForm.biaya.value = jumlah*price*jarak;    
    }
    function proses() {
    HargaTotal();
    }
    ...
    $('#modaledit').on('show.bs.modal', function (event) {
        ...
        modal.find('.modal-body #jarak').val(jarak);
